...
<template>
  <div class="setting-group">
    ...
    <div class="flex flex-col sm:flex-row gap-2 sm:gap-3 mb-2 sm:mb-3">
      ...
      >
        <PhDownload :size="18" class="sm:w-5 sm:h-5" /> {{ t('modal.opml.import') }}
      </button>
      <button
        class="btn-secondary flex-1 justify-center text-sm sm:text-base"
        @click="handleExportOPML"
      >
        <PhUpload :size="18" class="sm:w-5 sm:h-5" /> {{ t('modal.opml.export') }}
      </button>
    </div>
    <div class="flex mb-2 sm:mb-3">
      ...
      >
        <PhBroom :size="18" class="sm:w-5 sm:h-5" /> {{ t('setting.database.cleanDatabase') }}
      </button>
    </div>
  </div>
</template>
