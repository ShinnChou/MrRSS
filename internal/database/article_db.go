...
func (db *DB) SaveArticle(article *models.Article) error {
	db.WaitForReady()
	query := `INSERT OR IGNORE INTO articles (feed_id, title, url, image_url, audio_url, published_at, translated_title, content, is_read, is_favorite, is_hidden, is_read_later) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`
	_, err := db.Exec(query, article.FeedID, article.Title, article.URL, article.ImageURL, article.AudioURL, article.PublishedAt, article.TranslatedTitle, article.Content, article.IsRead, article.IsFavorite, article.IsHidden, article.IsReadLater)
	return err
}
...
func (db *DB) SaveArticles(ctx context.Context, articles []*models.Article) error {
	...
	stmt, err := tx.PrepareContext(ctx, `INSERT OR IGNORE INTO articles (feed_id, title, url, image_url, audio_url, published_at, translated_title, content, is_read, is_favorite, is_hidden, is_read_later) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`)
	if err != nil {
		return err
	}
	defer stmt.Close()
	...
	for _, article := range articles {
		...
		_, err := stmt.ExecContext(ctx, article.FeedID, article.Title, article.URL, article.ImageURL, article.AudioURL, article.PublishedAt, article.TranslatedTitle, article.Content, article.IsRead, article.IsFavorite, article.IsHidden, article.IsReadLater)
		if err != nil {
...
func (db *DB) GetArticles(filter string, feedID int64, category string, showHidden bool, limit, offset int) ([]models.Article, error) {
	db.WaitForReady()
	baseQuery := `
		SELECT a.id, a.feed_id, a.title, a.url, a.image_url, a.audio_url, a.content, a.published_at, a.is_read, a.is_favorite, a.is_hidden, a.is_read_later, a.translated_title, f.title
		FROM articles a
	...
	for rows.Next() {
		...
		var imageURL, audioURL, content, translatedTitle sql.NullString
		if err := rows.Scan(&a.ID, &a.FeedID, &a.Title, &a.URL, &imageURL, &audioURL, &content, &a.PublishedAt, &a.IsRead, &a.IsFavorite, &a.IsHidden, &a.IsReadLater, &translatedTitle, &a.FeedTitle); err != nil {
